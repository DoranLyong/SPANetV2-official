...
default_cfgs = {
    ...
    'spanetv2_s18_hybrid': _cfg(),
    'spanetv2_s36_pure': _cfg(),
    'spanetv2_s36_hybrid': _cfg(),
    'spanetv2_m36_pure': _cfg(),
    'spanetv2_m36_hybrid': _cfg(),
    'spanetv2_b36_pure': _cfg(),
    'spanetv2_b36_hybrid': _cfg(),    
}
...
class ExSPAM(nn.Module):
    ...
    def __init__(self, dim, num_heads=4, expand_ratio=2, act_layer=nn.GELU, bias=False,
        ...
        self.split_indexes = [self.split_groups for i in range(num_heads)]
        ...
        for i in range(self.num_heads):
            if i < 3:
                ...
                band_kernel_size = 3 + (i+1)*2
                local_conv = nn.Sequential(
                    nn.Conv2d(dim//self.num_heads, dim//self.num_heads, kernel_size=(1, band_kernel_size), padding=(0, band_kernel_size//2), stride=1, groups=dim//self.num_heads), # dwconv_w 
                    nn.Conv2d(dim//self.num_heads, dim//self.num_heads, kernel_size=(band_kernel_size, 1), padding=(band_kernel_size//2, 0), stride=1, groups=dim//self.num_heads)  # dwcon_h
                )
                setattr(self, f"local_conv_{i + 1}", local_conv)      
    ...
    def forward(self, x):
        ...
        head1, head2, head3, head4 = torch.split(self.ctx(x).permute(0,-1,1,2), self.split_indexes, dim=1)
        conv_feat = torch.cat((getattr(self, f"local_conv_{1}")(head1), getattr(self, f"local_conv_{2}")(head2),
                               getattr(self, f"local_conv_{3}")(head3), getattr(self, f"local_conv_{4}")(head4)),
            dim=1
        )

        if (H, W//2+1) != (self.size, self.filter_size):
            self.filter = resize_weight(self.filter,  H, W//2+1)     

        SRF = torch.sigmoid(self.filter)
        s_i = torch.fft.rfft2(conv_feat.to(torch.float32), dim=(2, 3), norm='ortho')
        s_i = s_i * SRF
        s_i = torch.fft.irfft2(s_i, s=(H, W), dim=(2, 3), norm='ortho').to(x.dtype)
        s_ctx = self.proj(s_i).permute(0, 2, 3, 1) # (B, H, W, C)
...
@register_model
def spanetv2_s18_hybrid(pretrained=False, **kwargs):
    model = SPANetV2(
        depths=[3, 3, 9, 3],
        dims=[64, 128, 320, 512],
        ...
        head_fn=MlpHead,
        **kwargs)
    model.default_cfg = default_cfgs['spanetv2_s18_hybrid']
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            url= model.default_cfg['url'], map_location="cpu", check_hash=True)
        model.load_state_dict(state_dict)
...
@register_model
def spanetv2_s36_pure(pretrained=False, **kwargs):
    model = SPANetV2(
        depths=[3, 12, 18, 3],
        dims=[64, 128, 320, 512],
        token_mixers=ExSPAM,
        head_fn=MlpHead,
        **kwargs)
    model.default_cfg = default_cfgs['spanetv2_s36_pure']
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            url= model.default_cfg['url'], map_location="cpu", check_hash=True)
        model.load_state_dict(state_dict)
...
@register_model
def spanetv2_s36_hybrid(pretrained=False, **kwargs):
    model = SPANetV2(
        depths=[3, 12, 18, 3],
        dims=[64, 128, 320, 512],
        token_mixers=[ExSPAM, ExSPAM, MixAttention, Attention],
        head_fn=MlpHead,
        **kwargs)
    model.default_cfg = default_cfgs['spanetv2_s36_hybrid']
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            url= model.default_cfg['url'], map_location="cpu", check_hash=True)
        model.load_state_dict(state_dict)
...
@register_model
def spanetv2_m36_pure(pretrained=False, **kwargs):
    model = SPANetV2(
        depths=[3, 12, 18, 3],
        dims=[96, 192, 384, 576],
        token_mixers=ExSPAM,
        head_fn=MlpHead,
        **kwargs)
    model.default_cfg = default_cfgs['spanetv2_m36_pure']
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            url= model.default_cfg['url'], map_location="cpu", check_hash=True)
        model.load_state_dict(state_dict)
...
@register_model
def spanetv2_m36_hybrid(pretrained=False, **kwargs):
    model = SPANetV2(
        depths=[3, 12, 18, 3],
        dims=[96, 192, 384, 576],
        token_mixers=[ExSPAM, ExSPAM, MixAttention, Attention],
        head_fn=MlpHead,
        **kwargs)
    model.default_cfg = default_cfgs['spanetv2_m36_hybrid']
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            url= model.default_cfg['url'], map_location="cpu", check_hash=True)
        model.load_state_dict(state_dict)
...
@register_model
def spanetv2_b36_pure(pretrained=False, **kwargs):
    model = SPANetV2(
        depths=[3, 12, 18, 3],
        ...
        head_fn=MlpHead,
        **kwargs)
    model.default_cfg = default_cfgs['spanetv2_b36_pure']
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            url= model.default_cfg['url'], map_location="cpu", check_hash=True)
        model.load_state_dict(state_dict)
...
@register_model
def spanetv2_b36_hybrid(pretrained=False, **kwargs):
    model = SPANetV2(
        depths=[3, 12, 18, 3],
        dims=[128, 256, 512, 768],
        token_mixers=[ExSPAM, ExSPAM, MixAttention, Attention],
        head_fn=MlpHead,
        **kwargs)
    model.default_cfg = default_cfgs['spanetv2_b36_hybrid']
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(
            url= model.default_cfg['url'], map_location="cpu", check_hash=True)
        model.load_state_dict(state_dict)
