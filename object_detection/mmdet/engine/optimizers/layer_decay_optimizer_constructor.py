...
def get_layer_id_for_spanetv2(var_name, max_layer_id):
    if var_name in ('backbone.cls_token', 'backbone.mask_token',
                    'backbone.pos_embed'):
        return 0
    elif var_name.startswith('backbone.downsample_layers'):
...
def get_stage_id_for_spanetv2(var_name, max_stage_id):
    if var_name in ('backbone.cls_token', 'backbone.mask_token',
                    'backbone.pos_embed'):
        return 0
    elif var_name.startswith('backbone.downsample_layers'):
...
class SPANetV2LearningRateDecayOptimizerConstructor(DefaultOptimWrapperConstructor):
    ...
    def add_params(self, params: List[dict], module: nn.Module,
        ...
        logger.info('Build SPANetV2LearningRateDecayOptimizerConstructor  '
        ...
        for name, param in module.named_parameters():
            ...
            if 'layer_wise' in decay_type:
                if 'spanetv2' in module.backbone.__class__.__name__:
                    layer_id = get_layer_id_for_spanetv2(
                ...
                if 'spanetv2' in module.backbone.__class__.__name__:
                    layer_id = get_stage_id_for_spanetv2(name, num_layers)
                    logger.info(f'set param {name} as id {layer_id}')
                else:
                    raise NotImplementedError()
